...
def main():
    p=argparse.ArgumentParser();p.add_argument('--features',required=True);p.add_argument('--output',required=True);p.add_argument('--dataset',choices=['DDSM','INbreast','pooled'],default='pooled');p.add_argument('--device',default='cuda' if torch.cuda.is_available() else 'cpu');p.add_argument('--seed',type=int,default=12345);p.add_argument('--outer-folds',type=int,default=5);p.add_argument('--inner-folds',type=int,default=3);p.add_argument('--pca-variance',type=float,default=.95);p.add_argument('--lr',type=float,default=1e-4);p.add_argument('--batch-size',type=int,default=16);p.add_argument('--weight-decay',type=float,default=1e-4);p.add_argument('--epochs',type=int,default=50);p.add_argument('--patience',type=int,default=5);p.add_argument('--dropout',type=float,default=.1);p.add_argument('--fuzzy-uncertainty',type=float,default=.2);p.add_argument('--membership-shift',type=float,default=0);p.add_argument('--rho',type=float,default=.85);p.add_argument('--beta',type=float,default=.5);p.add_argument('--art-alpha',type=float,default=1e-3);p.add_argument('--bootstrap',type=int,default=1000);a=p.parse_args();seed_everything(a.seed);d=load(a.features)
    if a.dataset!='pooled':
